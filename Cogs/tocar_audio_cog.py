...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, url, *, stream=False, ctx=None) -> list:
        ...
        if 'entries' in initial_data:
            ...
            sources.append(cls(discord.FFmpegPCMAudio(filename, **ffmpeg_options), data=initial_data))
            return sources
...
class Audio(commands.Cog):
    ...
    async def play(self, ctx: Context, url: str):
        ...
        def play_next(e=None):
            ...
            if len(self.players[str(ctx.guild.id)]) > 0:
                if not ctx.voice_client.is_playing():
                    ...
                    ctx.voice_client.play(player, after=lambda e: play_next(e))
        ...
        if self.current_player[str(ctx.guild.id)] is None:
            ...
            if not ctx.voice_client.is_playing():
                ctx.voice_client.play(music, after=lambda e: play_next(e))
                self.current_player[str(ctx.guild.id)] = music
            if not ctx.voice_client.is_playing():
                await ctx.send(f'Tocando **{music.title}**!!!!')
        else:
            print(f'[INFO] **{format(musics[0].title)}** adicionado a fila ({ctx.guild}[{ctx.guild.id}])')
