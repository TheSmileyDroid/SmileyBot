...
class RPG(commands.Cog):
    ...
    async def roll(self, ctx: commands.Context, *, args: str):
        ...
        if len(dice) == 0:
            await ctx.send("Formato inválido. Exemplo: 3d6+1 ou 3d6")
            return
        try:
            rolls = int(dice[0])
            limit = int(dice[1])
            ...
            await ctx.send(
                f"O formato deve ser NdN+N! Error when converting: {e}\
            \ndice: {dice}"
            )
            return
        ...
        result = (
            ...
            if len(numbers) > 1
            else "" + str(sum(int(n) for n in numbers) + modifier)
        )
        result += (
            f" ({str(sum(int(n) for n in numbers))} _**{modifier:+}**_)"
            if modifier
            else ""
        )
    ...
    @commands.command()
    async def flip(self, ctx, *, args: str) -> None:
        """Flips a coin."""
