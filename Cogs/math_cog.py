...
class Math(commands.Cog):
    ...
    @commands.command(name='sqrt', aliases=['raiz', 'raiz quadrada', 'raizquadrada', 'raizq'])
    async def sqrt(self, ctx: Context, a: int):
        await ctx.send(str(a ** 0.5))

    @commands.command(name='mod', aliases=['%', 'resto', 'resto da divisão', 'restodadivisão', 'restodivisão', 'resto da divisao', 'restodadivisao', 'restodivisao'])
    ...
    @commands.command(name='calcular', aliases=['calc', 'calculadora', 'calculate', 'cal'])
    async def calc(self, ctx: Context, *, args: str):
