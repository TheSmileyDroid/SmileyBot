...
```\n\
        ...
        if script == "list":
            await ctx.send(
                f'**Scripts:** {", ".join(self.scripts[ctx.author.id].keys())}'
            )
            return
        ...
        if args[0] == "new":
            ...
            await ctx.send(
                f"**Scripts:** Script {script_name} sendo editado. Use end para finalizar."
            )
    ...
    @commands.Cog.listener()
    async def on_message(self: commands.Bot, message: discord.Message):
        if message.author.id in self.editing:
            if message.content.startswith("-"):
                return
            if message.content == "end":
                await message.channel.send(
                    f"**Scripts:** Script {self.editing[message.author.id]} finalizado."
                )
