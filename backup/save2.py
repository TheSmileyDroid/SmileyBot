...
async def play(ctx: Context, url: str, channel: str = None):
    ...
    voice.play(video.audiostreams[0].url)
...
async def listen(ctx: Context, channel: str = None):
    ...
    voice.play("audio.mp3", after=lambda e: print(
        f"Exception: {0}".format(e)))


@bot.command()
async def pararConversa(ctx: Context):
    modo_conversa[0] = False
...
@bot.command()
async def modoConversa(ctx: Context, channel: str = None):
    voice = await get_voice_client(ctx, channel)

    modo_conversa[0] = True

    while modo_conversa[0]:
        voice.record(lambda e: print(f"Exception: {e}"))
        print("Started record")
...
async def falar(ctx: Context, text: str, channel: str = None, lang: str = "pt-br"):
    ...
    voice.play("audio.mp3", after=lambda e: print(f"Exception: {0}".format(e)))
...
async def get_voice_client(ctx, channel):
    voice_channel, connected, voice = get_voice_channel_and_status(
        ctx, channel)
    if not connected:
        voice: VoiceClient = await voice_channel.connect(cls=NativeVoiceClient)
